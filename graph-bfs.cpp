...
#include <cassert>
  ...
  // pre: from < n(), to < n()
  // post: returns a vector listing the ids of the vertices along the shortest path between vertex
  // 'from' and vertex 'to' std::vector<std::size_t> shortestPath(std::size_t from, std::size_t to);
  std::vector<std::size_t> Graph::shortestPath(std::size_t from, std::size_t to) {
    std::queue<std::size_t> q;
    std::vector<bool> visited(n(), 0); 
    std::vector<std::size_t> prev(n(), -1);

    dist[from] = 0; //change for shortest distance
    q.push(from); //adds first vertice to queue    

    while(q.size() > 0) {
      //Visit next vertice
      std::size_t currentV = q.front();
      q.pop();
      ...
      for(int i = 0; i < adjList_[currentV].size(); i++) {
        std::size_t neighborV = adjList_[currentV][i];
        q.push_back(neighborV);
        if(prev[neighborV] == -1) {
          prev[neighborV] = currentV;
        }
      }
    }
